...
paths=sorted(glob.glob(filepath+"*higgs*"))
print paths
...
for i in range(N):
 t.GetEntry(i)
 if t.fat_E.size()>=1 and  t.fat_BHad_n_GA[0]>=2:
  #print t.fat_assocTrkjet_ind[0],t.trkjet_BHad_n_GA
  trkcount=0
  for j in t.fat_assocTrkjet_ind[0]:
   if j<=t.trkjet_BHad_n_GA.size()-1:
    if t.trkjet_BHad_n_GA[j] > 0:
     trkcount=trkcount+1
  #print trkcount
  if trkcount>=2:
   if t.fat_assocTrkjet_ind[0][1]<=t.trkjet_DL1r_pb.size()-1 and t.fat_assocTrkjet_ind[0][1]<=t.trkjet_DL1r_pu.size()-1 and t.fat_assocTrkjet_ind[0][1]<=t.trkjet_DL1r_pc.size()-1:
    ...
    Weight[0]=mcEventWeight[0]*reweight
